...
import 'package:twodoteight/data/datasource/remote/dio/dio_client.dart';
import 'package:twodoteight/data/datasource/remote/exception/api_error_handler.dart';
...
class ApiIntegrationDemoRepo {
  ...
  Future<ApiResponse> getData({required String path}) async {
    try {
  ...
  Future<ApiResponse> updateData(int productID) async {
    try {
      final response = await dioClient.put('end_url_here');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
